...
OUT_DIR="${OUT_DIR:-out}"
# Normalise OUT_DIR to an absolute path. kbuild chdir()s into $(O) before
# running recipes, so any *relative* path handed to make (notably
# INSTALL_MOD_PATH) would be re-anchored inside the output directory and
# silently land in out/out/... instead of out/...
...
config_needs_regen() {
    ...
    # Sanity-check a few options that must survive the fragment merge with the
    # expected storage class. A mismatch means .config is stale relative to the
    # fragments (e.g. generated by an older build.sh that forced =y).
    # CONFIG_PINCTRL_MSM is deliberately a module here: sun_perf.config ships it
    # as =m and the platform loads it from vendor_dlkm like every other
    # Qualcomm target (pineapple/parrot/monaco).
    local key_configs=(
    ...
    for kc in "${key_configs[@]}"; do
        if ! grep -qxF "${kc}" "$config_file"; then
            return 0
        fi
    done
...
make_defconfig() {
    if config_needs_regen; then
        ...
        # -y: builtin (=y) takes precedence over module (=m) when both appear, so
        #     a device fragment can never silently demote a GKI builtin.
...
# Bundle everything needed to turn a crash report back into function names:
# the unstripped modules plus the kernel's own symbol table.
package_debug_symbols() {
    ...
    log "Packaging debug symbols into ${DEBUG_SYMBOLS_ARCHIVE}..."
    local staging="${OUT_DIR}/debug_symbols"
    rm -rf "${staging}"
    mkdir -p "${staging}"
    ...
    mkdir -p "${DIST_DIR}"
    rm -f "${archive_path}"
    tar -C "${staging}" -I 'zstd -T0' -cf "${archive_path}" .
    rm -rf "${staging}"

    [ -s "${archive_path}" ] || { error "Symbol archive is empty: ${archive_path}"; exit 1; }
    tar -tf "${archive_path}" >/dev/null || { error "Symbol archive is corrupt: ${archive_path}"; exit 1; }
    log "  Symbol archive created: ${archive_path} ($(du -h "${archive_path}" | cut -f1))"
}
...
install_modules() {
    ...
    for tree in "${install_trees[@]}"; do
        ...
        # Start clean so a previous install cannot leave stale modules behind.
        rm -rf "${OUT_DIR}/${tree}"
        set +e
        # shellcheck disable=SC2086 # strip_flag must expand to two argv entries
        make "${MAKE_ARGS[@]}" INSTALL_MOD_PATH="${OUT_DIR}/${tree}" ${strip_flag} modules_install >> "$mod_log" 2>&1
        mod_status=$?
        set -e
        if [ "$mod_status" -ne 0 ]; then
            show_build_errors "$mod_log"
            error "Module install into ${tree} failed! (exit code: ${mod_status})"
            exit 1
        fi

        # Remove symlinks and build artifacts
        rm -f "${OUT_DIR}/${tree}/lib/modules/"*/build
        rm -f "${OUT_DIR}/${tree}/lib/modules/"*/source
    done

    # Guard against silent module loss: a missing/empty install tree means the
    ...
    if [ "${debug_count}" -ne "${release_count}" ]; then
        error "Release module tree is incomplete: ${release_count} modules stripped, ${debug_count} built"
        exit 1
    fi
    ...
    log "  Installed ${release_count} modules (debug ${debug_size} bytes, release ${release_size} bytes)"
}
...
package_anykernel3() {
    ...
    mkdir -p "${DIST_DIR}"
    rm -f "${zip_path}"
    ( cd "${ANYKERNEL3_DIR}" && zip -r9 "${zip_path}" . -x ".git" ".git/*" "README.md" ".gitignore" )
