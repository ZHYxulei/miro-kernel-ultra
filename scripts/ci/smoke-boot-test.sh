...
#
# Every path given is booted in turn, so CI can prove that the Image *inside
# each published package* is bootable rather than a single build-tree copy.
# All of them are attempted even if an earlier one fails, and the script exits
# non-zero if any of them did not reach userspace.
#
# Environment:
#   SMOKE_TEST_TIMEOUT  seconds to wait for each guest (default 180)
...
# ---------------------------------------------------------------------------
# Boot one image and check what came up
# ---------------------------------------------------------------------------
dump_tail() {
...
# boot_image <image> <console log>
# Returns 0 only when the guest reached /init and completed, so the caller can
# keep testing the remaining images instead of stopping at the first failure.
boot_image() {
    local image="$1"
    local guest_log="$2"
    local qemu_status
    ...
    qemu_status=$?
    set -e
    # timeout(1) returns 124 when it had to kill the guest; the poweroff path
    # makes QEMU exit 0, but accept a killed guest as long as the log proves we
    # got there.
    ...
    if [ ! -s "${guest_log}" ]; then
        printf 'Smoke boot test failed: %s produced no console output\n' "${image}" >&2
        return 1
    fi
    if ! grep -q 'Linux version' "${guest_log}"; then
        dump_tail "${guest_log}"
        printf 'Smoke boot test failed: %s never printed a version banner\n' "${image}" >&2
        return 1
    fi
    if grep -q 'Kernel panic' "${guest_log}"; then
        dump_tail "${guest_log}"
        printf 'Smoke boot test failed: %s panicked during boot\n' "${image}" >&2
        return 1
    fi
    if grep -q 'Unable to handle kernel' "${guest_log}"; then
        dump_tail "${guest_log}"
        printf 'Smoke boot test failed: %s oopsed during boot\n' "${image}" >&2
        return 1
    fi
    if ! grep -q 'SMOKE-BOOT: userspace init reached' "${guest_log}"; then
        dump_tail "${guest_log}"
        printf 'Smoke boot test failed: %s did not reach userspace /init\n' "${image}" >&2
        return 1
    fi
    if ! grep -q 'SMOKE-BOOT-OK' "${guest_log}"; then
        dump_tail "${guest_log}"
...
for image in "${images[@]}"; do
    index=$((index + 1))
    guest_log="${work_dir}/console-${index}.log"
    log "Smoke boot test ${index}/${#images[@]}: ${image}"
    if boot_image "${image}" "${guest_log}"; then
        log "OK: ${image} booted to userspace"
    else
        status=1
    fi
done

[ "${status}" -eq 0 ] || fail "at least one image failed to boot"
log "All ${#images[@]} image(s) booted to userspace successfully"
